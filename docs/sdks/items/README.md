...
<!-- UsageSnippet language="typescript" operationID="envelope-item-download" method="get" path="/envelope/item/{envelopeItemId}/download" -->
```typescript
import { Documenso } from "@documenso/sdk-typescript";
...
```typescript
import { DocumensoCore } from "@documenso/sdk-typescript/core.js";
import { envelopesItemsDownload } from "@documenso/sdk-typescript/funcs/envelopesItemsDownload.js";
...
async function run() {
  const res = await envelopesItemsDownload(documenso, {
    envelopeItemId: "<id>",
  });
  if (res.ok) {
    const { value: result } = res;
    ...
    console.log("envelopesItemsDownload failed:", res.error);
  }
}
